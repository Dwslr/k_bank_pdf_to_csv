...
cols_to_drop = [col for col in df_ffin.columns if col.startswith('Unnamed:')]
df_ffin = df_ffin.drop(cols_to_drop, axis=1)

# Convert columns from text in float
df_ffin[['income', 'outcome', 'amount_curr']] = df_ffin[['income', 'outcome', 'amount_curr']].apply(lambda x: x.str.replace(r'\s', '', regex=True).str.replace(',', '.', regex=False).astype(float))
# Convert column in desired date format 
df_ffin[['date_op', 'date_deb']] = df_ffin[['date_op', 'date_deb']].apply(lambda x: pd.to_datetime(x, format='%d.%m.%Y %H:%M:%S').dt.strftime('%Y-%m-%d'))
...
df_ffin['date_deb'] = pd.to_datetime(df_ffin['date_deb'],format='%Y-%m-%d')
